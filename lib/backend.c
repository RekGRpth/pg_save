...
static void backend_connect_or_reset(Backend *backend) {
    ...
    const char *values[] = {backend->host, getenv("PGPORT") ? getenv("PGPORT") : DEF_PGPORT_STR, MyProcPort->user_name, MyProcPort->database_name, MyBgworkerEntry->bgw_type, backend->state <= state_primary ? "read-write" : "any", NULL};
